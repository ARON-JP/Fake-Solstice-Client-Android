...
#pragma endregion
                    ...
                    if (modRect.y > catRect.y + 0.5f)
                    {
                        ...
                        // Keybind display: show the bound key on the right side of the module.
                        // While binding this module, show "..." instead.
                        bool isBindingThis = (isBinding && mod == lastMod);
                        std::string bindName;
                        if (isBindingThis)
                            bindName = "...";
                        else if (mod->mKey != 0)
                            bindName = Keyboard::getKey(mod->mKey);

                        if (!bindName.empty())
                        {
                            float bindTextSize = textSize * 0.9f;
                            float bindNameLen = ImRenderUtils::getTextWidth(&bindName, bindTextSize);
                            float bindTextHeight = ImRenderUtils::getTextHeight(bindTextSize);

                            ImVec4 bindRect = ImVec4((modRect.z - 5.f) - bindNameLen - 7.f, modRect.y + 3.f, modRect.z - 5.f, modRect.w - 3.f);
                            float bindCentreY = bindRect.y + ((bindRect.w - bindRect.y) - bindTextHeight) / 2.f;
                            ImVec2 bindTextPos = ImVec2(bindRect.x + 3.5f, bindCentreY);

                            ImRenderUtils::fillRectangle(bindRect, ImColor(29, 29, 29), animation, 4);
                            ImRenderUtils::drawText(bindTextPos, bindName, ImColor(255, 255, 255), bindTextSize, animation, true);
                        }
                        ...
                        if (ImRenderUtils::isMouseOver(modRect) && catPositions[i].isExtended && isEnabled && catPositions[i].isExtended)
                        {
                            ...
                            {
                                tooltip = mod->mDescription + " (Middle-click to bind)";
                            }
